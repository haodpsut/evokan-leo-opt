...
def local_train(model, X, y, epochs=3, lr=0.01, mu=0.0, global_vec=None,
                l1=0.0, weight_decay=1e-4, batch=128, device="cpu", task="reg"):
    """Train a client model. task='reg' (MSE, scalar target) or 'cls' (CE).
    FedProx term if mu>0."""
    ...
    for _ in range(epochs):
        ...
        for i in range(0, n, batch):
            ...
            opt.zero_grad()
            out = model(X[idx])
            loss = F.mse_loss(out, y[idx]) if task == "reg" else F.cross_entropy(out, y[idx])
...
@torch.no_grad()
def predict_reg(model, X, device="cpu"):
    model.to(device).eval()
    X = torch.as_tensor(X, dtype=torch.float32, device=device)
    return model(X).view(-1).cpu().numpy()


@torch.no_grad()
def nmse(model, X, y, device="cpu"):
    """Normalized MSE of a regression model on (X, y)."""
    import numpy as np
    p = predict_reg(model, X, device)
