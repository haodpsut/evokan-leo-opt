...
\subsection{Scaling and required capacity}
Fig.~\ref{fig:scaling}(a) shows accuracy as the number of participating terminals
grows; the federated surrogate improves with more contributors. Fig.~
\ref{fig:scaling}(b) sweeps the KAN grid size: accuracy saturates at a modest grid
and does not benefit from a larger one, which explains why online grid
self-evolution is unnecessary here and a fixed, well-sized grid is preferred.
...
\end{axis}
\end{tikzpicture}}
\caption{(a) accuracy improves with more terminals; (b) accuracy saturates at a
modest grid size, so a static grid suffices.}
\label{fig:scaling}
\end{figure}
